# CCAR-F — Claude Certified Architect: Foundations

Yeh notes **Claude Certified Architect – Foundations Exam Guide v1.0** (effective July 2026, exam code
CCAR-F) ka full breakdown hain, PDF `Read` tool se poora parha gaya 2026-08-24. **28 Aug 2026 se yeh
FDE pair ka DOOSRA Anthropic exam hai** ([CCAO-F](../ccao-f/README.md) pehle, phir CCAR-F) — dekho
[Stage Two](../02-stage-two-anthropic.md). Panaversity-aligned free version:
[PCAR-F](../pcar-f/README.md) (same blueprint). CCAR-F Claude Partner Network tier eligibility mein
count hota hai (CCAO-F nahi). Book: "launched March 2026, programme ka pehla exam."

Source PDF: `docs/certifications/Claude+Certified+Architect+–+Foundations+Exam+Guide.pdf`

## Index

1. [00 — Quick Facts + Intended Audience](00-quick-facts-and-audience.md)
2. [01 — Domain Blueprint (Full Task Statements, 5 Domains)](01-domain-blueprint.md)
3. [02 — Scope, Scoring & Exam Format (6 Scenarios + In/Out-of-Scope)](02-scope-scoring-and-exam-format.md)
4. [03 — How to Prepare + 4 Exercises + 12 Sample Questions (Full)](03-how-to-prepare-and-sample-questions.md)
5. [04 — Policies, Resources & Document Control](04-policies-resources-and-doc-control.md)

## Poori Cheez Ek Line Mein

> CCAR-F recall test nahi karta — **judgment** test karta hai: jab agent 12% cases mein verification skip
> kare, jab tool descriptions overlap karein, jab review ek pass mein contradictory feedback de, sahi fix
> almost kabhi "prompt ko strict-er likho" nahi hota — sahi fix **programmatic enforcement** (hooks,
> prerequisites, focused passes, scoped tools) hota hai. Architect ka kaam probabilistic compliance ko
> deterministic guarantees mein badalna hai jahan galti ki cost zyada ho.

---
[⬅ Certifications Index](../README.md)
